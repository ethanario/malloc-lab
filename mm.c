...
/* More constants and macros */
//#define ALIGN(size) ((~0x7 & ((size) + (ALIGNMENT-1))))
...
static block_t *coalesce(block_t * block) 
{
	size_t prev_alloc = ((find_prev(block)->header) & 0x1);
	size_t next_alloc = ((find_next(block)->header) & 0x1);
	size_t size = get_size(block);
    
	if (prev_alloc && next_alloc){ // Both the previous and next block are allocated
		return block;
	}
	else if (prev_alloc && !next_alloc) { /* Only the previous block is allocated */
		size += get_size(find_next(block));
		block->header = pack(size, 0);
	}
	else if (!prev_alloc && next_alloc) { /* Only the next block is allocated */
		size += get_size(find_prev(block));
		find_prev(block)->header = pack(size, 0);
		block = find_prev(block);
	}
	else { //Neither previous or next block is allocated
